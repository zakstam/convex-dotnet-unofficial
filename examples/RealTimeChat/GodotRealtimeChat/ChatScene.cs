...
public partial class ChatScene : Control
{
    ...
    public override void _Ready()
    {
        ...
        _chatService.ErrorOccurred += ShowErrorToast;
        ...
        _usernameDropdown = GetNode<OptionButton>("%UsernameDropdown");
        ...
        // Enable input focus styling
        _messageInput.FocusEntered += () => AnimateInputFocus(true);
        _messageInput.FocusExited += () => AnimateInputFocus(false);
    ...
    {
        GD.Print($"[ChatScene] DisplayMessages() - Displaying {messages?.Count ?? 0} messages");
        ...
        bool preserveScrollPosition = _isLoadingOlderMessages || _shouldAddSeparator;
        float previousContentHeight = 0f;
        float previousScrollValue = 0f;
        int previousCount = _previousMessageCount;
        bool shouldAddSeparator = _shouldAddSeparator;
        ...
        var pageBoundaries = _chatService?.PageBoundaries ?? Array.Empty<int>();
    ...
    private void OnSearchTextChanged(string searchText)
    {
        ...
        foreach (Node child in _searchResults!.GetChildren())
        {
            child.QueueFree();
        }
