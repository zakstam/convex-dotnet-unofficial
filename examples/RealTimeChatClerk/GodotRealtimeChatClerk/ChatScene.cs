...
public partial class ChatScene : Control
{
    ...
    public override void _Ready()
    {
        ...
        // If Clerk is configured, hide username dropdown and show auth button
        var hasClerk = ConvexManager.Instance.ClerkTokenService != null ||
                      (!string.IsNullOrWhiteSpace(ConvexManager.Instance.ChatConfig?.ClerkPublishableKey) &&
                       ConvexManager.Instance.ChatConfig.ClerkPublishableKey != "pk_test_YOUR_CLERK_PUBLISHABLE_KEY_HERE");

        if (hasClerk)
        {
            GD.Print("[ChatScene] Clerk configuration detected, setting up Clerk UI");
            ...
            // Authentication check will happen when Clerk is ready (see OnClerkReady)
        }
        else
        {
            GD.Print("[ChatScene] No Clerk configuration, using username dropdown");
            // Use username dropdown if Clerk not configured
            if (_usernameDropdown != null)
            {
        ...
        GD.Print($"[ChatScene] About to check Clerk - ConvexManager ready: {ConvexManager.Instance != null}");
        ...
        // Check if Clerk is configured - check both ClerkTokenService and config
        var chatConfig = instance?.ChatConfig;
        var clerkKey = chatConfig?.ClerkPublishableKey ?? "";
        var hasClerkTokenService = instance?.ClerkTokenService != null;
        var hasClerkKey = !string.IsNullOrWhiteSpace(clerkKey) && clerkKey != "pk_test_YOUR_CLERK_PUBLISHABLE_KEY_HERE";
        var hasClerkConfig = hasClerkTokenService || hasClerkKey;

        GD.Print($"[ChatScene] Clerk check - TokenService: {hasClerkTokenService}, Key present: {hasClerkKey}, Key length: {clerkKey?.Length ?? 0}, Key starts with: '{clerkKey?.Substring(0, Math.Min(30, clerkKey?.Length ?? 0))}...'");

        if (hasClerkConfig)
        {
            GD.Print("[ChatScene] Clerk configuration detected - BLOCKING message loading until authenticated");

            // DO NOT load messages yet - wait for authentication
            // Subscribe to Clerk ready signal
            if (instance?.ClerkTokenService != null)
            {
                GD.Print("[ChatScene] ClerkTokenService already exists, subscribing to ClerkReady signal");
                _ = ConvexManager.Instance?.Connect("ClerkReady", new Callable(this, nameof(OnClerkReady)));
            }
            else
            {
                GD.Print("[ChatScene] ClerkTokenService not yet created, will wait for ClerkReady signal");
                // Connect to signal even if ClerkTokenService isn't ready yet
                _ = ConvexManager.Instance?.Connect("ClerkReady", new Callable(this, nameof(OnClerkReady)));
            }

            // Check auth immediately AND use a timer as backup
            GD.Print("[ChatScene] Checking auth immediately...");
            _ = CallDeferred(nameof(CheckClerkAuth));

            // Also use a timer as backup
            var timer = new Timer();
            timer.WaitTime = 0.5f; // Wait 500ms for Clerk to initialize
            timer.OneShot = true;
            timer.Timeout += () =>
            {
                GD.Print("[ChatScene] Timer expired, checking Clerk auth again");
                CheckClerkAuth();
                timer.QueueFree();
            };
            AddChild(timer);
            timer.Start();
            GD.Print("[ChatScene] Started backup timer to check Clerk auth in 0.5 seconds");
        }
        else
        {
            GD.Print("[ChatScene] No Clerk configured - BUT backend requires auth, so we'll show dialog on error");
            // Even if Clerk isn't configured in frontend, backend requires auth
    ...
    {
        GD.Print("[ChatScene] Clerk ready, checking authentication");
        CheckClerkAuth();
    }

    private void CheckClerkAuth()
    {
        GD.Print("[ChatScene] CheckClerkAuth called");

        // Check if Clerk is configured in the first place
        var config = ConvexManager.Instance.ChatConfig;
        var clerkKey = config?.ClerkPublishableKey ?? "";
        var hasClerkConfig = !string.IsNullOrWhiteSpace(clerkKey) && clerkKey != "pk_test_YOUR_CLERK_PUBLISHABLE_KEY_HERE";

        if (!hasClerkConfig)
        {
            GD.Print("[ChatScene] No Clerk configuration found, skipping auth check");
            return;
        }

        var tokenService = ConvexManager.Instance.ClerkTokenService;
        if (tokenService == null)
        {
            GD.Print("[ChatScene] ClerkTokenService is null, but Clerk is configured - showing auth dialog immediately");
            // Show auth dialog immediately - don't wait
            ShowAuthDialog();
            return;
        }

        GD.Print($"[ChatScene] IsAuthenticated: {tokenService.IsAuthenticated}");
    ...
    private void ShowAuthDialog()
    {
        ...
        if (tokenService == null)
        {
            GD.Print("[ChatScene] ClerkTokenService is null, checking if we can create it");
            var config = ConvexManager.Instance.ChatConfig;
            var clerkKey = config?.ClerkPublishableKey ?? "";

            GD.Print($"[ChatScene] Clerk key from config: '{clerkKey?.Substring(0, Math.Min(30, clerkKey?.Length ?? 0))}...'");

            // Even if it's the placeholder, create the service so user can enter token manually
            if (!string.IsNullOrWhiteSpace(clerkKey))
            {
                GD.Print("[ChatScene] Creating ClerkTokenService from config (even if placeholder)");
                var clerkOptions = new ClerkOptions
                {
                    PublishableKey = clerkKey, // Use whatever is in config
                    TokenTemplate = config?.ClerkTokenTemplate ?? "convex",
                    EnableTokenCaching = true,
                    TokenCacheExpiration = TimeSpan.FromMinutes(5)
                };
                tokenService = new GodotClerkTokenService(clerkOptions);
                // Store token service in ConvexManager (setter is now internal)
                ConvexManager.Instance.ClerkTokenService = tokenService;
            }
            else
            {
                GD.PrintErr("[ChatScene] Cannot show auth dialog - no Clerk configuration at all");
                // Show error toast instead
                ShowErrorToast("Clerk authentication is required but not configured. Please set Clerk:PublishableKey in appsettings.json");
                return;
            }
        }

        GD.Print("[ChatScene] Loading ClerkAuthDialog scene...");
        // Load ClerkAuthDialog scene
        var dialogScene = GD.Load<PackedScene>("res://ClerkAuthDialog.tscn");
        if (dialogScene == null)
        {
            GD.PrintErr("[ChatScene] ClerkAuthDialog.tscn not found. Creating dialog programmatically...");
            // Fallback: Create dialog programmatically
            CreateAuthDialogProgrammatically(tokenService);
            return;
        }

        GD.Print("[ChatScene] Instantiating ClerkAuthDialog...");
        _authDialog = dialogScene.Instantiate<ClerkAuthDialog>();
        if (_authDialog == null)
        {
            GD.PrintErr("[ChatScene] Failed to instantiate ClerkAuthDialog - using fallback");
        ...
        _authDialog.Connect(ClerkAuthDialog.SignalName.AuthenticationSucceeded, new Callable(this, nameof(OnAuthenticationSucceeded)));
        _authDialog.Connect(ClerkAuthDialog.SignalName.AuthenticationFailed, new Callable(this, nameof(OnAuthenticationFailed)));

        GD.Print("[ChatScene] Initializing dialog...");
        _authDialog.Initialize(tokenService);

        GD.Print("[ChatScene] Showing ClerkAuthDialog...");
        _authDialog.PopupCentered();
    ...
    private void CreateAuthDialogProgrammatically(GodotClerkTokenService tokenService)
    {
        GD.Print("[ChatScene] Creating auth dialog programmatically...");

        // Create a simple dialog programmatically as fallback
        var dialog = new Window();
        dialog.Title = "Sign In with Clerk";
        ...
        var label = new Label();
        label.Text = "Clerk authentication is required.\n\nPlease enter your Clerk token manually:";
        label.AutowrapMode = TextServer.AutowrapMode.WordSmart;
        vbox.AddChild(label);

        var tokenInput = new LineEdit();
        tokenInput.PlaceholderText = "Paste your Clerk token here";
        tokenInput.SizeFlagsHorizontal = Control.SizeFlags.ExpandFill;
        vbox.AddChild(tokenInput);
        ...
        signInButton.Pressed += () =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                try
                {
                    tokenService.SetTokenManually(token);
                    GD.Print("[ChatScene] Token set manually");
                    OnAuthenticationSucceeded();
                    dialog.QueueFree();
                }
                catch (ConvexException ex)
                {
                    GD.PrintErr($"[ChatScene] Failed to set token: {ex.Message}");
                    ShowErrorToast($"Failed to set token: {ex.Message}");
                }
            }
        };
    ...
    {
        var tokenService = ConvexManager.Instance.ClerkTokenService;
        if (tokenService == null || !tokenService.IsAuthenticated)
        {
            _userLabel!.Text = "User: Not authenticated";
            return;
        }

        // For now, just show authenticated status
        // In a full implementation, you might fetch user info from Clerk
        if (_userLabel != null)
        {
            _userLabel.Text = "User: Authenticated";
        }

        // Update chat service username if needed
        if (_chatService != null)
        {
            _chatService.Username = "Authenticated User";
        }
    }
