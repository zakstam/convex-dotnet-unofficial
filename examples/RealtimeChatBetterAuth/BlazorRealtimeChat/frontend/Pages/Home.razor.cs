...
public partial class Home : ComponentBase, IDisposable
{
    ...
    [Inject] public IBetterAuthService AuthService { get; set; } = default!;
    [Inject] public IConvexClient ConvexClient { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.ChatService ChatService { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.PresenceService PresenceService { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.ReactionService ReactionService { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.FileService FileService { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.ReplyService ReplyService { get; set; } = default!;
    [Inject] public RealtimeChat.Shared.Services.ReadReceiptService ReadReceiptService { get; set; } = default!;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        // If already authenticated, initialize the chat
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        else if (AuthService.IsAuthenticated && !_hasScrolledAfterLogin)
        {
            await ScrollToBottom();
    ...
    private async void OnMessagesUpdated(List<MessageDto> messages)
    {
        ...
        await InvokeAsync(async () =>
        {
            ...
            // IMMEDIATELY update state with new messages for optimistic updates
            // This must happen before any async calls to ensure instant UI feedback
            State.Messages = messages.ToList();
            State.CanLoadMore = ChatService.HasMoreMessages;
            StateHasChanged();

            // Now perform async operations (scroll position check, attachments, reactions)
            // These won't block the initial UI update
            ...
            if (State.Messages.Count > 0 && AuthService.IsAuthenticated)
            {
