...
public class ChatService : IDisposable
{
    ...
    /// <summary>
    /// Event raised when a reply is sent (for optimistic updates in reply threads).
    /// </summary>
    public event Action<MessageDto>? ReplyAdded;

    /// <summary>
    /// Send a reply to a message with optimistic updates.
    /// The reply appears immediately in the UI, then is confirmed by the server.
    /// If the mutation fails, the optimistic update is rolled back automatically.
    /// </summary>
    public async Task SendReplyAsync(string text, MessageId parentMessageId, List<Attachment>? attachments = null)
    {
        ...
        // Create an optimistic reply that will appear immediately
        var optimisticReply = new MessageDto
        {
            Id = $"optimistic-reply-{Guid.NewGuid()}",
            Username = Username,
            Text = messageText,
            Timestamp = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
        ...
        try
        {
            _ = await _client.Mutate<object>(_sendMessageFunctionName)
                ...
                .Optimistic(default(object)!, _ =>
                {
                    ...
                    appliedOptimistic = true;
                    ReplyAdded?.Invoke(optimisticReply);

                    // Also add to current messages if it's a top-level view that shows replies
                    _currentMessages.Add(optimisticReply);
                    MessagesUpdated?.Invoke(_currentMessages);
                })
                .WithRollback(() =>
                {
                    if (appliedOptimistic)
                    {
                        _currentMessages.RemoveAll(m => m.Id == optimisticReply.Id);
                        MessagesUpdated?.Invoke(_currentMessages);
                    }
                })
                .ExecuteAsync();
    ...
    /// <summary>
    /// Edit an existing message with optimistic updates.
    /// The edit appears immediately in the UI, then is confirmed by the server.
    /// If the mutation fails, the optimistic update is rolled back automatically.
    /// </summary>
    public async Task EditMessageAsync(string messageId, string newText)
    {
        ...
        // Find the original message for rollback
        var originalMessage = _currentMessages.FirstOrDefault(m => m.Id == messageId);
        if (originalMessage == null)
        {
            ErrorOccurred?.Invoke("Message not found");
            return;
        }
        ...
        try
        {
            _ = await _client.Mutate<object>(_editMessageFunctionName)
                ...
                .Optimistic(default(object)!, _ =>
                {
                    ...
                    var message = _currentMessages.FirstOrDefault(m => m.Id == messageId);
                    if (message != null)
                    {
                        var index = _currentMessages.IndexOf(message);
                        _currentMessages[index] = new MessageDto
                        {
                            Id = message.Id,
                            Username = message.Username,
                            Text = trimmedText,
                            Timestamp = message.Timestamp,
                            EditedAt = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
    ...
    /// The message disappears immediately from the UI, then is confirmed by the server.
    /// If the mutation fails, the optimistic update is rolled back automatically.
    /// </summary>
    public async Task DeleteMessageAsync(string messageId)
    {
        ...
        // Store the message for potential rollback
        var deletedMessage = _currentMessages.FirstOrDefault(m => m.Id == messageId);
        var deletedIndex = deletedMessage != null ? _currentMessages.IndexOf(deletedMessage) : -1;
        var appliedOptimistic = false;
        ...
        try
        {
            _ = await _client.Mutate<object>(_deleteMessageFunctionName)
                ...
                .Optimistic(default(object)!, _ =>
                {
                    ...
                    var removed = _currentMessages.RemoveAll(m => m.Id == messageId);
                    if (removed > 0)
                    {
                        appliedOptimistic = true;
                        MessagesUpdated?.Invoke(_currentMessages);
                ...
                .WithRollback(() =>
                {
                    if (appliedOptimistic && deletedMessage != null)
                    {
                        // Restore the deleted message
                        if (deletedIndex >= 0 && deletedIndex <= _currentMessages.Count)
                        {
                            _currentMessages.Insert(deletedIndex, deletedMessage);
                        }
                        else
                        {
                            _currentMessages.Add(deletedMessage);
                        }
                        MessagesUpdated?.Invoke(_currentMessages);
                    }
                })
