...
public class ReadReceiptService : IReadReceiptService, IDisposable
{
    ...
    private readonly HashSet<string> _pendingReadMutations = [];
    ...
    /// <summary>
    /// Gets a deep clone of MessageReadReceipts for optimistic updates.
    /// </summary>
    private Dictionary<string, List<MessageReadDto>> ClonedReadReceipts
    {
        get
        {
            var cloned = new Dictionary<string, List<MessageReadDto>>();
            foreach (var kvp in MessageReadReceipts)
            {
    ...
    /// <summary>
    /// Mark a message as read with optimistic updates.
    /// The read receipt appears immediately in the UI, then is confirmed by the server.
    ...
    public async Task MarkMessageAsReadAsync(string messageId, string username)
    {
        ...
        if (MessageReadReceipts.TryGetValue(messageId, out var existingReads))
        {
            if (existingReads.Any(r => r.Username == username))
            {
                _logger?.LogDebug("MarkMessageAsReadAsync: Message {MessageId} already marked as read by {Username}", messageId, username);
                return;
            }
        }
        ...
        try
        {
            ...
            await _convexClient.Mutate<object>(_markMessageReadFunctionName)
                ...
                .OptimisticWithAutoRollback(
                    getter: () => ClonedReadReceipts,
                    setter: value =>
                    {
                        MessageReadReceipts = value;
                        ReadReceiptsUpdated?.Invoke(this, EventArgs.Empty);
                    },
                    update: receipts =>
                    {
                        var updated = new Dictionary<string, List<MessageReadDto>>(receipts);

                        if (!updated.TryGetValue(messageId, out var readsList))
                        {
                            readsList = [];
                            updated[messageId] = readsList;
                        }

                        // Only add if not already present
                        if (!readsList.Any(r => r.Username == username))
                        {
                            readsList.Add(new MessageReadDto
                            {
                                Username = username,
                ...
                .TrackPending(_pendingReadMutations, messageId)
